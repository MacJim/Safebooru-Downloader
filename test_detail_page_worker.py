...
class DetailPageWorkerTestCase (unittest.TestCase):
    ...
    @staticmethod
    def get_image_detail_page_urls_queue():
        image_detail_page_urls_queue = Queue()
        for test_case in DetailPageWorkerTestCase.TEST_CASES:
        ...
        return image_detail_page_urls_queue

    def test_2_urls(self):
        image_detail_page_urls_queue = DetailPageWorkerTestCase.get_image_detail_page_urls_queue()

        with tempfile.TemporaryDirectory() as dir_name:
            detail_page_worker.detail_page_worker(image_detail_page_urls_queue, dir_name, set())
    ...
    def test_ignored_images(self):
        ignored_images = set(test_case[IMAGE_ID_KEY] for test_case in self.TEST_CASES)

        image_detail_page_urls_queue = DetailPageWorkerTestCase.get_image_detail_page_urls_queue()

        with tempfile.TemporaryDirectory() as dir_name:
            detail_page_worker.detail_page_worker(image_detail_page_urls_queue, dir_name, ignored_images)

            for test_case in self.TEST_CASES:
                self.assertFalse(file_helper.image_exists(test_case[IMAGE_ID_KEY], dir_name))
